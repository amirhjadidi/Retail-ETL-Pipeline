...
def transform_data(sales_data, stores_date, features_date):
    ...
    features_date['Date'] = pd.to_datetime(features_date['Date'], format='mixed', dayfirst=True)
    ...
    sales_data['Date'] = pd.to_datetime(sales_data['Date'], format='mixed', dayfirst=True)
